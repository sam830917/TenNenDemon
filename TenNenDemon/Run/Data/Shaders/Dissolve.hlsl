...
{
   float3 BURN_START_COLOR;		// color at the start of the burn 
   float BURN_EDGE_WIDTH; 		// defines the width of the burn edge
   
   float3 BURN_END_COLOR;
   float BURN_AMOUNT;			// 0 is nothing, 1 is fully gone
};
...
Texture2D <float4> tDissolvePattern   : register(t5);
...
float4 FragmentFunction( v2f_t input ) : SV_Target0
{
	...
	final_color = lerp( final_color, burn_color, 1.f - burn_mix );
