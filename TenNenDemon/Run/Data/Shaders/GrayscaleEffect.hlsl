...
cbuffer gray_scale_contants : register(b5)
{
	float INTENSITY;
	float3 padding00;
};
...
float4 FragmentFunction( VertexToFragment_t input ) : SV_Target0 // semeantic of what I'm returning
{
   ...
   float greyscaleAverage = (color.x + color.y + color.z)/3.0f;
   float r = lerp( color.x, greyscaleAverage, INTENSITY );
   float g = lerp( color.y, greyscaleAverage, INTENSITY );
   float b = lerp( color.z, greyscaleAverage, INTENSITY );
