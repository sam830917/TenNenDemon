...
{
	float textureAspect = m_spriteSheet.GetTexture().GetAspect();
	float uvWidth = fabsf( m_uvAtMaxs.x - m_uvAtMins.x );
	float uvHeight = fabsf( m_uvAtMaxs.y - m_uvAtMins.y );
	return textureAspect * ( uvWidth / uvHeight );
}
